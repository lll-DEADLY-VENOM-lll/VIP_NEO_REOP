    ...
    try:
        ...
        if callback_query.message.chat.username:
            try:
                ...
                await callback_query.message.reply_text(f"Failed to join: {e}")
        else:
            try:
                invitelink = await client.export_chat_invite_link(chat_id)
                await userbot.join_chat(invitelink)
                await callback_query.message.reply_text(
                    "**Assistant Joined successfully via invite link✅**"
                )
            except Exception as e:
                await callback_query.message.reply_text(f"Failed to join: {e}")
    except Exception as e:
        await callback_query.answer(
            f"Failed to unban: Make sure I am admin with ban power.",
            show_alert=True,
        )
...
def PlayWrapper(command):
    async def wrapper(client, message):
        ...
        if PRIVATE_BOT_MODE == str(True):
            if not await is_served_private_chat(message.chat.id):
                await message.reply_text(
                    "**ᴘʀɪᴠᴀᴛᴇ ᴍᴜsɪᴄ ʙᴏᴛ**\n\nᴏɴʟʏ ғᴏʀ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴄʜᴀᴛs. ᴀsᴋ ᴏᴡɴᴇʀ ᴛᴏ ᴀʟʟᴏᴡ ʏᴏᴜʀ ᴄʜᴀᴛ."
                )
        ...
        # --- Assistant Join Logic (FIXED LINE 194 ERROR) ---
        if not await is_active_chat(chat_id):
            ...
            try:
                ...
                if get.status in [ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED]:
                    return await message.reply_text(
                        text=f"**Assistant is banned in this group.**\n\n**ID:** `{userbot.id}`\n**Username:** @{userbot.username}",
                ...
                if message.chat.username:
                    try:
                        ...
                        return await message.reply_text(f"Assistant join fail: {e}")
                else:
                    try:
                        ...
                        return await message.reply_text("I need 'Invite Users' permission to bring assistant.")
                    except Exception as e:
                        return await message.reply_text(f"Assistant join fail: {e}")
            except Exception:
                pass
